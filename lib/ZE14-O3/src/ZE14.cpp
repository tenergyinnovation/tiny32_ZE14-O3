...
uint8_t ZE14::calculateChecksum(uint8_t msg[kMessageLength])
{
    uint8_t bytesum = 0;
...
ZE14::ZE14(HardwareSerial &device) : hw_stream_(&device)
{
    setStream();
}

ZE14::ZE14(SoftwareSerial &device) : sw_stream_(&device)
{
    setStream();
}

float ZE14::getActiveData()
{
    ...
    { // make sure that there is at least one message in the buffer
        ...
        for (int8_t i = (sizeof(serial_buffer) - 1); i >= 0; i--)
        {
            ...
            if ((serial_buffer[i] == 0xFF) && (serial_buffer[i + 1] == 0x2A) && (serial_buffer[i + 2] == 0x04) && ((i + kMessageLength - 1) <= static_cast<uint8_t>(sizeof(serial_buffer)))) //<-- Change ID from 0x03 to 0x04
        ...
        */
        ...
        if ((message_buffer[0] == 0xFF) && (message_buffer[1] == 0x2A) && (message_buffer[2] == 0x04) && (message_buffer[8] == calculateChecksum(message_buffer))) //<-- Change ID from 0x03 to 0x04
        {
